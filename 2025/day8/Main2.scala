...
class AocApp extends App :
  ...
  var lastBox1 : JuncBox = null
  var lastBox2 : JuncBox = null

  // NOTE: use 10 for test data
  // NOTE: use 1000 for first assignment
  // NOTE: use 1000_000 for second assignment
  val n_iter_max = 1000_000
  var n_iter = 1
  while (n_iter <= n_iter_max)
      ...
      if (bestBox1 != null && bestBox2 != null) then
          ...
          // rememeber last connection
          lastBox1 = bestBox1
          lastBox2 = bestBox2

          var cirSize : Int = bestBox1.getCircuit().size
          println("  MERGED: circuit size = " + cirSize)
          if (cirSize == jboxArray.size)
              println()
              println("  ALL MERGED: circuit size = " + cirSize)
              println("  LAST BOXES: " + lastBox1.toName() + " - " + lastBox2.toName())
              println("  MULT-X: " + lastBox1.getXin() * lastBox2.getXin())
              n_iter = n_iter_max
          
      else
          // EXIT
          println()
          println("  <BOXES were NULL> ")
          println("  LAST BOXES: " + lastBox1.toName() + " - " + lastBox2.toName())
          n_iter = n_iter_max
  ...
  for (w <- jboxArray)
      ...
      if (n_box != n_cir)
          println("WARNING: w[" + bi + "]: " + n_box + " boxes == set.size " + n_cir + " SHOULD BE SAME")
      bi += 1
  ...
  */
  ...
  var prod_box : Int = 0
  if (mySizes.size > 3) then
      prod_box = mySizes(0) * mySizes(1) * mySizes(2)
  else
      println("Too few sizes!!")
  println()
  println("Prod-Box = " + prod_box)
